...
global lastByUID := {}   ; UID -> counts snapshot
...
Class NexusTK {
	...
	static SPELL_OFFSET			:= 0x18E378 	; incremented 12/27/2020
	static SPELL_OFFSET_2			:= 0x18E378 ;
	static DIRECTION_OFFSET			:= 0x27A748 ;
	static GROUP_SIZE_OFFSET		:= 0x27A748 ;
	static CHAT_OFFSET			:= 0x2DD4A4 ; unused?
	static TALK_READ_OFFSET			:= 0x27A764	;
	static STATUS_OFFSET_1			:= 0x27A874	;
	static STATUS_OFFSET_2			:= 0x29AE0C ;
	static MACRO_OFFSET_1			:= 0x2FE1B8 ; May be: 0x?????? for some
	static MACRO_OFFSET_2			:= 0x358	; May be: 0x??? for some
	static MAP_NAME_OFFSET			:= 0x29B4B4 ;
	static STAT_OFFSET				:= 0x29B4E4 ;
	static MOB_LIST_OFFSET			:= 0x29B89C ;
	static NAME_OFFSET				:= 0x29BEE0 ;
	static GROUP_LIST_OFFSET		:= 0x29BF3C ;
	static SPELL_LIST_OFFSET		:= 0x29BF40 ;
	static MOB_MEM_LOCATION_OFFSET	:= 0x2FE630 ; unused?
	static GROUND_LIST_OFFSET       := 0x29B9B4 ;
	static MENU_OPEN_OFFSET       	:= 0x69B380 ;
	static CREATION_MENU_OFFSET     := 0x20C ;
	static TINY_DELAY				:= 30
	static SMALL_DELAY				:= 60
	static MEDIUM_DELAY				:= 300
	static LARGE_DELAY				:= 600

	;; Sage / Worldshout / System / Whispers / Clan chat / Subpath chat
	static SYSTEM_READ_ADDRESSES 	:= [0x185CF0]
	static SYSTEM_READ_INDEX		:= 1

	static MACRO_RANGE				:= 30
	...
	static T_SCREEN_REFRESH			:= 180000
	...
	__new(program:="ahk_exe NexusTK.exe") {
		...
		;MsgBox % "Base Address (hex): " . Format("0x{:X}", this.baseAddress)
	...
	isMobInvisible2(mobIndex, offset_1=0x19E) {
    ...
    ;; mobIndex: 0 is the first mob in list
    mobAddr := offset_1 + this.mobSlotArray[mobIndex]

    invisibleByte := this.nexusMemory.read(mobAddr, "UChar")

    if (invisibleByte == 2) {
        ;; Turn invisible uninivisble yay!
        this.nexusMemory.write(mobAddr, 5, "UChar")
        return true
    }
	...
	getMobActiveSpells(mobIndex, offset_1:=0x178, offset_2:=0x10, uidOffset:=0x148) {
		...
		if (numSpells) {
			base := offset_1 + this.mobSlotArray[mobIndex]
			for idx in rangeincl(1, numSpells) {
				graphicId := this.nexusMemory.read(base, "Short", offset_2, multiplyHex(0x04, idx - 1), uidOffset)

				ids.Push(graphicId)

				if (!counts.HasKey(graphicId))
					counts[graphicId] := 0
				counts[graphicId] += 1
		...
		return {"scourged":scourged, "slept":slept, "poisoned":poisoned, "numSpells":numSpells, "ids":ids, "counts":counts}
	}



	getMobActiveSpellsz(mobIndex, offset_1=0x178, offset_2=0x10, uidOffset=0x148) {
		numSpells := this.getMobNumGraphics(mobIndex)
		slept := false
		scourged := false
		poisoned := false
		...
		return {"scourged":scourged, "slept":slept, "poisoned":poisoned, "parad":para}
	}
	...
	isMobTargeted(mobIndex, offset_1=0x1EC) {
		;MsgBox % "the mobSlotArray: " . this.mobSlotArray[mobIndex]
		targetBit := this.nexusMemory.read(offset_1 + this.mobSlotArray[mobIndex], "UChar")
		Return targetBit
	}

	isPlayerTargeted(playerName) {
		mob := this.getTargetedMob()
		if (mob) {
			return mob["name"] == playerName
		}
		else {
			return False
		}
	...
	compileMonsterInfo() {
		...
		validMobs := this.getValidMobs()
		For index, mobIndex in validMobs {
			;MsgBox % "i: " . index . " mi: " . mobIndex
			monster := this.getMobInfo(mobIndex)
			if (monster["UID"] == 0 and monster["xCoord"] == 0 and monster["xCoord"] == 0) {
				continue
			}

			;if (monster["name"] != "") {
			if (this.isPlayer("", mobIndex)) {
				;MsgBox % "PLAYER: " . monster["name"] . " - uid: " . monster["UID"] . "PlayerX: " . monster["xCoord"] . "`nPlayerY: " . monster["yCoord"]
			} else {
				;MsgBox % "MOB: " . monster["name"] . " - uid: " . monster["UID"] . "PlayerX: " . monster["xCoord"] . "`nPlayerY: " . monster["yCoord"]
			}
	...
	checkForPlayerAtCoordinate(xCoord, yCoord, playerInfo=False) {
        if (!playerInfo) {
            playerInfo := this.compilePlayerInfo()
        }
        For index, player in playerInfo {
	...
	tryMove(targetX, targetY) {
		...
		moveDir := this.determineBestMoveDirection(myX, myY, targetX, targetY)
			...
			If (!(myX == newX) || !(myY == newY)) {
				this.sendKeyStroke(this.SCREEN_REFRESH)
			}
		}
	}
...
;	}
	...
	getGroupMember(groupIndex, offset=0x220, maxVitaOffset=0x118, vitaOffset=0x11C, maxManaOffset=0x120, manaOffset=0x124) {
		...
		;MsgBox % "Name: " member.name
        ;. "`nVita: " member.vita " / " member.maxVita
        ;. "`nMana: " member.mana " / " member.maxMana
		Return member
	}
	...
	getNumberOfItems(itemName) {
	    ...
	    getGroundItems(validOffset:=0x4, xCoordOffset:=0x104, yCoordOffset:=0x108, xDrawCoordOffset:=0x10C, yDrawCoordOffset:=0x110, uidOffset:=0x12C) {
        allocationSlots := this.getGroundAllocationSlots()

        groundItems := []
        For index, slot in allocationSlots {
            valid := this.nexusMemory.read(slot + validOffset, "UInt")
		...
		If (envelopeMacro != "u") {
			If (A_TickCount - this.LT_AUTO_ENVELOPE > this.T_AUTO_ENVELOPE) {
				...
				If (exp > EXP_ENVELOPE_DELAY) {
					...
					this.sendKeyStroke(envelopeMacro)
				}
	...
	;----------------------------------------
	menuIsOpen() {
		isOpen := this.nexusMemory.read(this.MENU_OPEN_OFFSET, "UChar")
		return isOpen
	}
	...
	;----------------------------------------
	getMapId(offset_1:=0x27A764, offset_2:=0x3F2) {
		...
		for idx, waypoint in waypoints {
			...
			If (waypoint.direction != -1) {
				...
				if (servX == waypoint.x and servY == waypoint.y) {
					this.sendKeyStroke(waypoint.direction . waypoint.direction)
				} else {
