...
Class MageSpells Extends Spells {
	...
	__new(macroList, spellList, alignment:="") {
		...
		;; N_INFERNO :: Inferno
		If (alignment == this.KWI_SIN) {
			this.N_INFERNO := "Deaths door"
		} Else If (alignment == this.MING_KEN) {
			this.N_INFERNO := "Natures denial"
		} Else If (alignment == this.OHAENG) {
			this.N_INFERNO := "Steel storm"
		} Else {
			this.N_INFERNO := "Inferno"
		}
		
		;; N_TRANCE :: Mass Trance
		this.N_TRANCE := "Mass trance"
		...
		If (macroList) {
			...
			For idx in range(spellList.Length()) {
				For jdx in range(macroList.Length()) {
					...
					;; S_INFERNO
					If (!this.S_INFERNO and spellList[idx]["spell"] == this.N_INFERNO and spellList[idx]["slot"] == macroList[jdx]["slot"] and macroList[jdx]["type"] == this.M_TYPE_SPELL) {
						this.S_INFERNO := this.createKeyStroke(macroList[jdx]["num"], macroList[jdx]["mode"])
					}
					;; S_TRANCE
					If (!this.S_TRANCE and spellList[idx]["spell"] == this.N_TRANCE and spellList[idx]["slot"] == macroList[jdx]["slot"] and macroList[jdx]["type"] == this.M_TYPE_SPELL) {
						this.S_TRANCE := this.createKeyStroke(macroList[jdx]["num"], macroList[jdx]["mode"])
					}
				}
			}
		}
	...
	__new(macroList, spellList, alignment:="") {
		...
		For idx in range(spellList.Length()) {
			For jdx in range(macroList.Length()) {
				...
				;; S_TAUNT
				If (!this.S_TAUNT and spellList[idx]["spell"] == this.N_TAUNT and spellList[idx]["slot"] == macroList[jdx]["slot"] and macroList[jdx]["type"] == this.M_TYPE_SPELL) {
					this.S_TAUNT := this.createKeyStroke(macroList[jdx]["num"], macroList[jdx]["mode"])
				}
				;; S_SLASH
...
class Mobs {
	...
	__new() {
		...
		;; Dog 3 LR
		this.addMob(2064402,   "Killer Mongrel", true, "Dark amber" ) ;; Killer Mongrel
		this.addMob(1146975,   "Wolf-kin", true, "Dark amber" ) ;; Wolf-kin
		;; Bird 4                                 
		...
		this.addMob(164462,  "Frenetic Grizzly",  true, "Yellow amber, White amber, Amber, Dark amber" ) ;; Brown Bear
		this.addMob(1409646, "Magnetized Grizzly",true, "Yellow amber, White amber, Amber, Dark amber" ) ;; Black Bear
		this.addMob(33390,   "Engrossed Grizzly", true, "Yellow amber, White amber, Amber, Dark amber" ) ;; Blue Bear
		this.addMob(2065006, "Mesmeric Grizzly",  true, "Yellow amber, White amber, Amber, Dark amber" ) ;; Red Bear
		this.addMob(1213038, "Aklak",                  true ) ;; Boss Bear
		;; Blaze Training Grounds (Flametongue)
		this.addMob(1999367,"Flametongue Colonel",  true ) ;; Yellow Flametongue
		this.addMob(885255, "Flametongue Sergeant", true ) ;; Light Brown Flametongue
		this.addMob(819719, "Flametongue Master",   true ) ;; Dark Brown Flametongue
		this.addMob(688647, "Flametongue Corporal", true ) ;;  Green Flametongue
		this.addMob(754184, "Admiral Blaze",        true ) ;; Boss Bear
		;; Omphalos Training Grounds (Deers)
		this.addMob(33398,   "Nettled Saseum",   true ) ;; Brown Deer
		this.addMob(557686,  "Turbulent Saseum", true ) ;; Green Deer
		this.addMob(295542,  "Galled Saseum",    true ) ;; Blue Deer
		this.addMob(492150,  "Furious Saseum",   true ) ;; Purple Deer
		this.addMob(1409654, "Omphalos",         true ) ;; Boss Deer
		;; Gri-Yal Training Grounds (Dragons)
		this.addMob(164362,  "Blue Dragon",      true )
		this.addMob(229899,  "Yellow Dragon",    true )
		this.addMob(295435,  "Pink Dragon",      true )
		this.addMob(754186,  "White Dragon",     true )
		this.addMob(33292,   "Boss",             true )
		;; Woodlands Lobster
		this.addMob(1540264, "Deep sea lobster", true ) ;; Dark Gray Lobster
		this.addMob(1605874, "Sand fly",         true, "Yellow amber, Fly leg, Crystal fly eye, Fly wings, Seaweed")
		this.addMob(557224,  "Surf lobster",      true, "Yellow amber, Seaweed, Grain of sand")
		this.addMob(295080,  "Coral lobster",     true, "Yellow amber, Seaweed, Grain of sand")
		this.addMob(557224,  "Woodland lobster",  true, "Yellow amber, Seaweed, Grain of sand")
		;; Rooster
		this.addMob(32905,  "Rooster defender", true, "Amber, Chicken meat, Egg, Tao stone") 
		this.addMob(1998969,  "Sun feather", true, "Chicken meat, Egg") 
		;; Lost Mines
		  ; Squirrels
		this.addMob(360473,  "8 Carat Squirrel", true, "Silver acorn, Copper acorn, Sturdy mold") ;; Brown Squirrel
		this.addMob(1998873,  "24 Carat Squirrel", false, "Silver acorn, Copper acorn, Sturdy mold") ;; Gold Squirrel
		this.addMob(1933337,  "Silvered Squirrel", true, "Silver acorn, Copper acorn, Sturdy mold") 
		this.addMob(1671193,  "Squirrel Corporal", true) ;; Orange Squirrel
		  ; Rabbits
		this.addMob(1933333,  "Silvered Rabbit", true, "Lean rabbit meat, Tough rabbit meat")
		this.addMob(98435,  "8 Carat Rabbit", true, "Lean rabbit meat, Tough rabbit meat") ;; Rabbit Sentry
		  ; Nuts
		this.addMob(33796,  "Runaway Hazelnut", false, "Hazelnut")
		this.addMob(33795,  "Runaway Peanut", false, "Peanut")
		this.addMob(33799,  "Runaway Walnut", false, "Walnut")
		  ; Gnomes
		this.addMob(886448,  "Dunaz Gnome", false, "Sturdy mold, Empty bowl, Magically-sealed note")
		this.addMob(427697,  "Dunaz Gnomide", false, "Sturdy mold, Empty bowl, Magically-sealed note")
		this.addMob(2000560,  "Clion Gnome", false, "Sturdy mold, Empty bowl, Magically-sealed note")
		this.addMob(2000561,  "Clion Gnomide", false, "Sturdy mold, Empty bowl, Magically-sealed note")
		this.addMob(558768,  "Tende Gnome", false, "Sturdy mold, Empty bowl, Magically-sealed note")
		this.addMob(886449,  "Tende Gnomide", false, "Sturdy mold, Empty bowl, Magically-sealed note")
		  ; Trolls
		
	}

	addMob(uid, name:="", aggro:=false, drops:=false) {
	    if (drops) {
			drops := StrSplit(drops, ",", " ")
		    drops := this.getDrops(drops)
		}
		myDict := { "uid": uid, "name": name, "aggro": aggro, "drops":drops }
		this.MOBS.push(myDict)
		this.MOBSbyNAME[name] := myDict
		this.MOBSbyUID[uid] := myDict
	}
	
	getDrops(names) {
	    Drops := []
		for index, value in names {
			item := this.ITEMS.getItemFromName(value)
			if (item) {
			    Drops.push(item)
			}
		}
	...
	getMobFromName(name) {
		if (this.MOBSbyNAME.HasKey(name)) {
			return this.MOBSbyNAME[name]
		}
	}

	getMobFromUid(uid) {
		if (this.MOBSbyUID.HasKey(uid)) {
			return this.MOBSbyUID[uid]
		}
	}
	
	getMobNameFromUid(uid) {
		if (this.MOBSbyUID.HasKey(uid)) {
			return this.MOBSbyUID[uid]["name"]
		}
		return ""
	}
	
	getMobNamesFromUids(uids) {
		names := []
		for index, uid in uids {
			if (this.MOBSbyUID.HasKey(uid)) {
				names.push(this.MOBSbyUID[uid]["name"])
			} else {
				names.push(uid)
			}
		}
	...
	getMobUidFromName(name) {
		if (this.MOBSbyNAME.HasKey(name)) {
			return this.MOBSbyNAME[name]["uid"]
		}
		return ""
	}
...
Class Items {
	...
	static ITEMSbyNAME := []
	static ITEMSbyUID := {}
	static ITEMSbySUBCLASS := {}
	__new() {
		;; Craftable Items
		  ;; Ambers
		  this.addItem(49581, "Amber", "amber", 100)
		  this.addItem(49470, "Dark amber", "amber", 100)
		  this.addItem(49580, "White amber", "amber", 100)
		  this.addItem(49912, "Yellow amber", "amber", 20)
		  
	    ;; General items
		this.addItem(50284, "Seaweed", "misc", 25)
		this.addItem(49365, "Chicken meat", "misc", 30)
		this.addItem(49982, "Egg", "misc", 12)
		this.addItem(49922, "Tao stone", "misc", 1)
		
		;; Woodlands Lobster
		  ;; Sand fly
		  this.addItem(50151, "Fly leg", "woodlands_general", 3)
		  this.addItem(52466, "Crystal fly eye", "woodlands_general", 5)
		  this.addItem(51591, "Fly wings", "woodlands_general", 3)
		  ;; grain of sand
		  this.addItem(51367, "Grain of sand", "woodlands_general", 40)
		  
	}
	
	addItem(uid, name:="", subclass:="misc", maxstack:=1) {
		myDict := {"uid": uid, "name":name, "subclass":subclass, "maxstack":maxstack}
		this.ITEMS.push(myDict)
		this.ITEMSbyNAME[NAME] := myDict
		this.ITEMSbyUID[uid] := myDict
		if (!this.ITEMSbySUBCLASS.HasKey(subclass)) {
			this.ITEMSbySUBCLASS[subclass] := []
		}
		this.ITEMSbySUBCLASS[subclass].push(myDict)
	}
	
	
	getItemFromName(name) {
		if (this.ITEMSbyNAME.HasKey(name)) {
	...
	getItemUIDFromName(name) {
		if (this.ITEMSbyNAME.HasKey(name)) {
			return this.ITEMSbyNAME[name]["uid"]
		}
		return ""
	}
	
	getItemFromUID(uid) {
		if (this.ITEMSbyUID.HasKey(uid)) {
			return this.ITEMSbyUID[uid]
		}
	}
	
	getItemNameFromUID(uid) {
		if (this.ITEMSbyUID.HasKey(uid)) {
			return this.ITEMSbyUID[uid]["name"]
		}
		return ""
	}
